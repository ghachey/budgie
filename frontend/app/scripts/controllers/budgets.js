...
angular.module('pippDataApp.controllers.budgets', ['ui.bootstrap', 'ngAnimate', 'legendDirectives'])
  .controller('BudgetCtrl', ['$scope', '$location', '$routeParams', 'BudgetFactory', function ($scope, $location, $routeParams, BudgetFactory) {
    ...
    $scope.currentPalette = palettes[thisLevel];
    ...
    $scope.getPalette = function (){
      return $scope.currentPalette;
    };

    $scope.colorFunction = function() {
      return function(d, i) {
    	return $scope.currentPalette[i];
      };
    };
	...
	if (step !== 'categories'){
      ...
      $scope.currentPalette =  palettes[depth];
    };
...
angular.module('pippDataApp.controllers.one-off-charts', ['ui.bootstrap', 'ngAnimate', 'legendDirectives'])
  .controller('oneOffChartsCtrl', ['$scope', function ($scope) {
    ...
    $scope.currentPalette = 	    
      [
	'#ef8a62', '#67a9cf', '#1f77b4', '#aec7e8', '#ff7f0e', '#ffbb78', 
    ...
    $scope.vuDonorChartHeader = vuDonorVsGovtSpending[$scope.whichDonorChart].graph;
    $scope.vuDonorVsGovt      = vuDonorVsGovtSpending[$scope.whichDonorChart].series;

    $scope.nextDonorChart = function(index){
      if (typeof(index) === 'number' && index >= 0 && index <= vuDonorVsGovtSpending.length){
	...
	$scope.whichDonorChart++;
      ...
      $scope.whichDonorChart    = $scope.whichDonorChart >= vuDonorVsGovtSpending.length ? 0 : $scope.whichDonorChart;
      $scope.vuDonorVsGovt      =  vuDonorVsGovtSpending[$scope.whichDonorChart].series;
      $scope.vuDonorChartHeader = vuDonorVsGovtSpending[$scope.whichDonorChart].graph;
    ...
    $scope.vuEducationGraphHeader = vuEducationTrends[$scope.whichEducationGraph].graph;
    $scope.vuEducationGraph =  vuEducationTrends[$scope.whichEducationGraph].series;

    $scope.nextEducationGraph = function(index){
      if (typeof(index) === 'number' && index >= 0 && index <= vuEducationTrends.length){
	...
	$scope.whichEducationGraph++;
      ...
      $scope.whichEducationGraph = $scope.whichEducationGraph >= vuEducationTrends.length ? 0 : $scope.whichEducationGraph;
      $scope.vuEducationGraph =  vuEducationTrends[$scope.whichEducationGraph].series;
      $scope.vuEducationGraphHeader = vuEducationTrends[$scope.whichEducationGraph].graph;
    ...
    $scope.lineChartTooltips = function(key, x, y, e, graph) {
      ...
      var spendingStatus = gap < 0 ? 'deficit' : 'surplus';
    ...
    $scope.overspendChartTooltips = function(key, x, y, e, graph) {
      ...
      return '<h3>' + key + '</h3>' +
	'<p>' + int2roundKMG((parseFloat(y) * 1000000).toString()) + ' VATU<br />in ' + x + '</p>' +
	'<p>(' + percentOverspend + '%'   + ' overspent)</p>';
    };
	...
	var searchArray = $scope.vuDonorVsGovt[e.seriesIndex].values;

	for (var i = 0; i < searchArray.length; i++){
	  if (searchArray[i][0] === x && searchArray[i][1].toLocaleString('en') + '.0' === y){
	    var otherIndex = e.seriesIndex === 0 ? 1 : 0;
	    total = searchArray[i][1] + $scope.vuDonorVsGovt[otherIndex].values[i][1];
...
angular.module('pippDataApp.controllers.npps', ['ui.bootstrap', 'ngAnimate'])
  .controller('NPPCtrl', ['$scope', '$location', '$routeParams', 'BudgetFactory', function ($scope, $location, $routeParams, BudgetFactory) {
    ...
    $scope.currentYear = $routeParams.year ? $routeParams.year : 2014;
    var currentDocument = 'vu-npps-' + $scope.currentYear.toString();

    $scope.vuNPPs              = [];
    $scope.budgetCurrency     = 'VATU';
    $scope.currencyMultiplier = 1;
    $scope.annualTotal        = '';
    ...
    $scope.radioModel =  $scope.currentYear.toString();
